 ...
 */
@ManagedBean(name = "cierreEvalComAProg")
@ViewScoped
public class CierreEvalComProgManagedBean extends BaseJSFManagedBean implements Serializable {
    ...
    // </editor-fold> 
    public CierreEvalComProgManagedBean() {
        this.setPaginaMant("/pages/evaluacion/comite/cierreComAPrograma");
    }
    ...
    private void recuperaProgramaCierre(Long anio) {
        ...
        sql.append(" DECLARE @MATEST TABLE ( ANIO INT,CODIGO_ESP VARCHAR(4), NUM_EST INT)"
                ...
                + " WHERE ANIO = ").append(smciclo).append(""
                        ...
                        + " WHERE ANIO = ").append(smciclo).append(""
                        + " SELECT DISTINCT (case when enccal.anio is null then concat(materia.anio,'-',materia.codigo_esp)else concat(ENCCAL.anio,'-',ENCCAL.codigo_esp, '-', ENCCAL.CODIGO_ENCUESTA)end )cod, "
                        ...
                        + " isnull (ENCCAL.CODIGO_ENCUESTA,22) codigo_encuesta, "
                        ...
                        + " ISNULL(ENCCAL.tipo_encuesta, 'O') tipo_encuesta, "
                        ...
                        + "                         and mat.CODIGO_COORDINADOR  = MATERIA.CODIGO_COORDINADOR) ,1))*100),2) PORC, "
                        ...
                        + " where  MATERIA.ANIO = ").append(smciclo).append(""
                        ...
                        + " AND (ENCCAL.tipo_encuesta = 'O' or ENCCAL.tipo_encuesta is null) "
                        + " order bY FECHA_FIN");
